...
export class add {
	...
	public selectTel(type): void{
		...
		this.navCtrl.push(telephone,{telType:type,addType:this.addType});
	}
  ...
  public addContact(): void{
    ...
    this.addService.createContact(option,false).then(response =>{
        this.addService.createBase({"OwnerId": this.data.basicInfo.id || response.id}, telephoneOptions).then(base => {
          ...
          this.addService.saveContactsToServe(this.options, false).then(
            res => {
              ...
              this.navCtrl.push(detail, {'detailType': "edit"});
              for (let i of this.navCtrl.getViews()) {
                if (i.name === 'add' || i.name === 'detail' || i.name === 'telephone') {
      ...
      this.tsService.error(this.translation.addOneMoreTel)
    }
  }
  ...
  private init(): void{
    ...
    if(this.addType ===0&&this.navParams.get('telType')!=0&&this.navParams.get('telType')!=1){
      ...
      if(this.contactDetail){
        ...
        if(!this.data.basicInfo.firstName){
          this.data.basicInfo.firstName = ''
        }else if(!this.data.basicInfo.familyName){
          this.data.basicInfo.familyName = '';
        }
